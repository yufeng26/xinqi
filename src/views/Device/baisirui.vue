    ...
    <div class="selects">
      ...
      <div class="selecount">
        <el-select  :clearable="true" v-model="topicID" placeholder="请选择" >
          <el-option   v-for="item in topic" :key="item.ID" :label="item.tp_TopicName" :value="item.ID"></el-option>
        </el-select>


      </div>
    </div>
    <button @click="handleUserList" type="button" style="background:#006fe5 ;margin-left: 25%;" class="layui-btn layui-btn-normal">开始检索</button>
  </div>
    <div class="bigbox">
      <div class="consult">
        <button type="button" style="background:#01c8e7 ;" @click="handerPLExport" class="layui-btn layui-btn-normal"  v-if="menuModel.exportUsable">批量导出</button>
        <button type="button" style="background:#ff433f ;" @click="handlePLDelete"  class="layui-btn layui-btn-normal" v-if="menuModel.deleteUsable">批量删除</button>
        <div class="tab">
          <el-table style="width: 100%;"
                    :data="userList"  ref="multipleTable"
          ...
          >
            ...
            <el-table-column label="操作" >
              <template slot-scope="scope">
                ...
                <el-button @click.native.prevent="exportDevice(scope.row)" type="primary" v-if="menuModel.exportUsable">{{menuModel.export}}</el-button>
...
<script>
  ...
  export default {
    ...
    mounted(){
      var ddd=[{Name:"维度名称1",Score:12.5,Analysis:'结果分析',Proposal:'建议'}]
    ...
    methods:{
      ...
      handleUserList() {
        ...
        // param.append('gameId',this.topicID )
        ...
        this.userList = this.$ReportOptionAPI.GetBaisiruiList(param, function (data) {
              console.log(data);
              
                // let lstAtt=data.Result[3].lstAtt
                // JSON.parse(lstAtt).forEach(c=>{
                //   console.log(c)
                // })
          if (data.Code == 1) {
